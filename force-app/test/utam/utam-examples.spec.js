...
describe('utam-examples', () => {
  ...
  it('navigate to the accounts tab in the sales app and create an account record', async () => {
        ...
        for (const row of rows) {
          ...
          for (const item of items) {
            ...
            if (dropdownFields.has(fieldName)) {
              await chooseDropdown(item, value);
            } else if (baseInputFields.has(fieldName)) {
              await enterBaseInputField(item, value);
            } else if (inputFields.has(fieldName)) {
              await enterInputField(item, value);
            } else if(textAreaFields.has(fieldName)) {
              await enterTextField(item, value);
            }
          }
        }
      }
    ...
    await fillFormFields(recordLayout, fieldData);
    ...
    async function enterBaseInputField(item, value) {
    ...
    async function enterTextField(item, value) {
      const lightningInput = await item.getInput();
      await lightningInput.setText(value);
    }
