...
import { AppLauncherMenu, RecordActionWrapper, ObjectHome, DesktopLayoutContainer, InputField } from './pageObjects/index.js';
...
describe('utam-examples', () => {
  ...
  it('navigate to the accounts tab in the sales app and create an account record', async () => {
    ...
    for (const [labelText, value] of Object.entries(fieldData)) {
      for (const section of sections) {
        const sectionTitle = await section.getSectionTitle();
        const text = await sectionTitle.getText();
        console.log(text);
        const rows = await section.getRows();
        for (const row of rows) {
          const items = await row.getItems();
          for (const item of items) {
            const matchingItem = await item.getLabelText();
            let fieldValue = fieldData[matchingItem];
            let propertyExists = fieldData.hasOwnProperty(matchingItem);
            if(propertyExists) {
              switch (matchingItem.replaceAll(' ', '_').toUpperCase()) {
                case 'RATING':
                case 'OWNERSHIP':
                case 'BANKING':
                case 'SHIPPING_COUNTRY':
                case 'CUSTOMER_PRIORITY': {
                  await chooseDropdown(item, fieldValue);
                  break;
                }
                case 'ACCOUNT_NAME':
                case 'PHONE':
                ...
                case 'EMPLOYEES':
                case 'BILLING_STREET':
                case 'SLA_SERIAL_NUMBER':
                case 'DESCRIPTION': {
                  /* const baseInput = await item.getLightningInputWrapper();
                   const lightningInput = await baseInput.getInput();
                   await lightningInput.setText(fieldValue);*/
                  await enterInputField(item, fieldValue);
                  break;
                }
              }
            }
    ...
    async function chooseDropdown(item, value) {
    ...
    async function enterInputField(item, value) {
      const baseInput = await item.getLightningInputWrapper();
      const lightningInput = await baseInput.getInput();
