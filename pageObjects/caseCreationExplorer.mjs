...
async function _utam_get_statusItem(driver, root) {
    let _element = await _utam_get_generatedLayout(driver, root);
    const _locator = _By.css("records-record-layout-item[field-label='Status']");
    _element = new _ShadowRoot(driver, _element);
    return _element.findElement(_locator);
}
...
async function _utam_get_statusWrapper(driver, root) {
    ...
    const _locator = _By.css("support-lwc-input-case-status");
    return _element.findElement(_locator);
}
 ...
 * @author UTAM
 */
export default class CaseCreationExplorer extends _UtamBaseRootPageObject {
    ...
    async getStatusLightningPicklist() {
        ...
        element = await _createInstance(_Picklist, driver, element);
        await element.__beforeLoad__();
        return element;
    }
