...
async function _utam_get_listbox(driver, root) {
    let _element = root;
    const _locator = core.By.css("div[role='listbox'][aria-label='Suggestions']");
...
async function _utam_get_optionRoot(driver, root, _resultItemElementsIndex) {
    ...
    const _locator = core.By.css("div[role='option']");
    _elements = new core.ShadowRoot(driver, _elements);
...
async function _utam_get_primaryLabel(driver, root, _resultItemElementsIndex) {
    let _elements = await _utam_index_resultItemElements(driver, root, _resultItemElementsIndex);
    const _locator = core.By.css(".instant-result-item__content span[title]");
    _elements = new core.ShadowRoot(driver, _elements);
    const hasElement = await _elements.containsElement(_locator);
    if (!hasElement) { return null; }
    return _elements.findElement(_locator);
 ...
 * @author UTAM
 */
class GlobalSearchResults extends core.UtamBaseRootPageObject {
    constructor(driver, element, locator = core.By.css("search_dialog-instant-results-list")) {
...
module.exports = GlobalSearchResults;
